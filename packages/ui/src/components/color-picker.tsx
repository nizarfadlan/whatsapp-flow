...
function formatInputValue(format: PickerFormat, hex: string, rgb: Rgb) {
...
function parseRgbInput(raw: string): Rgb | null {
	const [r, g, b, ...rest] = raw.split(",").map((part) => Number(part.trim()));
	if (
		rest.length > 0 ||
		r === undefined ||
		g === undefined ||
		b === undefined ||
		[r, g, b].some((value) => Number.isNaN(value))
	) {
		return null;
	}
...
function parseHslInput(raw: string): Rgb | null {
	const [h, s, l, ...rest] = raw
		.replaceAll("%", "")
		.split(",")
	...
	if (
		...
		s === undefined ||
		l === undefined ||
		[h, s, l].some((value) => Number.isNaN(value))
	) {
		return null;
	}
			...
			<PopoverTrigger
				render={
					<Button
						type="button"
						variant="outline"
						className="h-10 w-full max-w-[320px] justify-start gap-2 px-3 font-normal"
					/>
				}
			>
				<span
					className="size-5 rounded-md border shadow-sm"
					style={{ backgroundColor: toRgbaString(rgb, alpha) }}
				/>
			...
			<PopoverContent align="start" className="w-[320px] p-0">
				<div className="space-y-3 p-3">
					...
					<div className="space-y-2">
						<div
							className="relative h-4 overflow-hidden rounded-full border"
							style={checkerboard}
						>
							<input
								type="range"
					...
					<div className="grid grid-cols-[auto_1fr_auto] items-center gap-2">
						...
						<Input
							value={formatInputValue(format, hex, rgb)}
							onChange={(event) => {
								...
								if (format === "hex") {
									...
									setHsv(next);
									setAlpha(showAlpha ? parsedHex.a : 1);
									emit(next, showAlpha ? parsedHex.a : 1);
									return;
								}

								const nextRgb =
									format === "rgb" ? parseRgbInput(raw) : parseHslInput(raw);
								if (!nextRgb) return;
						...
						<Select
							value={format}
							onValueChange={(value) => setFormat(value as PickerFormat)}
						>
						...
						{swatches.map((swatch) => (
							<button
								...
								onClick={() => {
									...
									setHsv(next);
									setAlpha(showAlpha ? parsedSwatch.a : 1);
									emit(next, showAlpha ? parsedSwatch.a : 1);
								}}
